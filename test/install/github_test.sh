...
test_extract_json_value() {
  fixture=./test-fixtures/github-api-docker-sbom-cli-plugin-v0.1.0-release.json
  content=$(cat ${fixture})

  actual=$(extract_json_value "${content}" "tag_name")
  assertEquals "v0.1.0" "${actual}" "unable to find tag_name"
...
test_github_release_tag() {
  ...
  assertEquals "v0.1.0" "${actual}" "unable to find release tag"
}
