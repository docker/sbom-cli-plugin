...
func getFixtureImage(t testing.TB, fixtureImageName string) string {
	...
	img := imagetest.GetFixtureImage(t, "docker", fixtureImageName)
	require.NotNil(t, img)
	require.NotEmpty(t, img.Metadata.Tags)
	return img.Metadata.Tags[0].String()
}
...
func getSyftBinaryLocationByOS(t testing.TB, goOS string) string {
	...
	switch goOS {
	case "darwin", "linux":
		return path.Join(repoRoot(t), fmt.Sprintf("snapshot/%s_%s_%s/%s", internal.ApplicationName, goOS, runtime.GOARCH, internal.BinaryName))
	default:
